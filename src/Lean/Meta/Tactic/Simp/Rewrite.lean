  ...
  let r : DischargeResult ← withTraceNode `Meta.Tactic.simp.discharge (fun
    ...
    if ctx.dischargeDepth >= ctx.maxDischargeDepth then
      ...
      | some proof =>
        ...
        return .proved
      | none =>
        --TODO make sure (← getMethods).discharge? == dischargeDefault?
        if isEqnThmHypothesis' type then
         let abstractType := (<- abstractMVars (Expr.lit (.strVal "noCache")))
...
def dischargeDefault? (e : Expr) : SimpM (Option Expr) := do
  ...
  if isEqnThmHypothesis e then
    ...
    if let some r ← dischargeEqnThmHypothesis? e then
      return some r
  let r ← simp e
  trace[Meta.Tactic.simp.negativeCache] "simp discharchge {e} {repr e}"
  if r.expr.isTrue then
    trace[Meta.Tactic.simp.negativeCache] "simp discharchge => true"
    return some (← mkOfEqTrue (← r.getProof))
  else
    trace[Meta.Tactic.simp.negativeCache] "simp discharchge => {r.expr} {repr r.expr}"
    return none
